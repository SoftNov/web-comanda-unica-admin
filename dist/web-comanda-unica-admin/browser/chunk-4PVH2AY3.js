  ...
  onSubmitCode() {
    ...
    this.authService.verifyResetCode({ email, code }).subscribe({
      next: (response) => {
        ...
        this.startCountdown(parseApiDate(response.resetTokenExpiresAt) ?? /* @__PURE__ */ new Date(), "resetToken");
      },
      error: (error) => {
        this.isSubmitting.set(false);
        if (error.status === 410) {
  ...
  requestCode(email) {
    ...
    this.authService.forgotPassword({ email }).subscribe({
      next: (response) => {
        ...
        this.codeForm.reset();
        this.step.set("code");
        this.startCountdown(parseApiDate(response.codeExpiresAt) ?? /* @__PURE__ */ new Date(), "code");
      },
      error: (error) => {
        this.isSubmitting.set(false);
        this.handleError(error, {
...
(() => {
  (typeof ngDevMode === "undefined" || ngDevMode) && \u0275setClassDebugInfo(PasswordResetFlowComponent, { className: "PasswordResetFlowComponent", filePath: "src\\app\\features\\auth\\components\\password-reset-flow\\password-reset-flow.component.ts", lineNumber: 25 });
})();

export {
  PasswordResetFlowComponent
};
//# sourceMappingURL=chunk-4PVH2AY3.js.map
